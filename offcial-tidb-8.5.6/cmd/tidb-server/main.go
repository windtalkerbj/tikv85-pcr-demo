...
// createReadOnlyDomain builds a Domain with schema loading but no DDL ownership.
// Used in PCR replication mode where the target cluster's writes are blocked
// but schema must stay current with KV data replicated from the source.
func createReadOnlyDomain(storage kv.Storage) *domain.Domain {
	ses, err := session.CreateSession(storage)
	if err != nil {
		log.Warn("PCR: failed to create session for read-only domain", zap.Error(err))
		return nil
	}
	dom := domain.GetDomain(ses)
	if dom == nil {
		log.Warn("PCR: failed to get domain from read-only session")
		return nil
	}
	dom.StartSchemaLoad()
	log.Info("PCR: read-only domain started (schema loading, no DDL)")
	return dom
}
...
func createStoreDDLOwnerMgrAndDomain(keyspaceName string) (kv.Storage, *domain.Domain) {
	...
	// PCR mode: target cluster is in replication, writes are blocked.
	// Tolerate DDL owner registration and bootstrap failures so TiDB can
	// start in read-only mode and serve queries on PCR-replicated data.
	...
	if err != nil {
		...
		if strings.Contains(errStr, "PCR replication mode") ||
			...
			log.Warn("PCR: DDL owner registration skipped, target in replication mode", zap.Error(err))
		} else {
			terror.MustNil(err)
		}
	}
	...
	if err != nil {
		...
		if strings.Contains(errStr, "PCR replication mode") ||
			strings.Contains(errStr, "writes not allowed") {
			log.Warn("PCR: bootstrap writes skipped, running read-only", zap.Error(err))
