...
func runInBootstrapSession(store kv.Storage, ver int64) {
	...
	if err != nil {
		// Bootstrap fail will cause program exit.
		if strings.Contains(err.Error(), "PCR replication mode") ||
				strings.Contains(err.Error(), "writes not allowed") {
				logutil.BgLogger().Warn("PCR: domain start skipped (writes blocked)", zap.Error(err))
			} else {
	...
	case ddl.Normal:
		...
		if err := InitMDLVariable(store); err != nil {
			if strings.Contains(err.Error(), "PCR replication mode") ||
				strings.Contains(err.Error(), "writes not allowed") {
				logutil.BgLogger().Warn("PCR: metadata lock init skipped (writes blocked)", zap.Error(err))
			} else {
...
func finishBootstrap(store kv.Storage) {
	...
	if err != nil {
			...
			if strings.Contains(errStr, "PCR replication mode") ||
				...
				logutil.BgLogger().Warn("PCR: finish bootstrap skipped, target in replication mode", zap.Error(err))
				return
			}
