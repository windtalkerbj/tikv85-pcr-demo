...
// Used in PCR read-only mode: writes are blocked but schema must stay
// current with KV data replicated from the source cluster.
// Unlike Start(), this only uses the ticker (no etcd syncer channels),
// since the etcd connection may fail under write protection.
func (do *Domain) StartSchemaLoad() {
	do.wg.Run(func() {
		do.loadSchemaTickerOnly(do.ctx)
	}, "loadSchemaInLoop")
}

// loadSchemaTickerOnly reloads the schema on a timer, without any etcd/syncer
// dependency. Used in PCR read-only mode where etcd writes are blocked.
// Uses 5s interval (vs default schemaLease/2 ~22s) to reduce DDL visibility delay.
func (do *Domain) loadSchemaTickerOnly(ctx context.Context) {
	defer util.Recover(metrics.LabelDomain, "loadSchemaInLoop", nil, true)
	ticker := time.NewTicker(5 * time.Second)
	defer func() {
		ticker.Stop()
		logutil.BgLogger().Info("loadSchemaInLoop (ticker-only) exited.")
	}()
	for {
		select {
		case <-ctx.Done():
