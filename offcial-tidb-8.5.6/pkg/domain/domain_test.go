...
func TestStartSchemaLoadDoesNotPanic(t *testing.T) {
	s, err := mockstore.NewMockStore()
	require.NoError(t, err)
	dom := NewDomain(s, 1*time.Second, 0, 0, nil)
	defer dom.Close()

	// Init must be called first (like the real path via CreateSession → domap.Get)
	require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/pkg/ddl/NoDDLDispatchLoop", `return(true)`))
	require.NoError(t, failpoint.Enable("github.com/pingcap/tidb/pkg/domain/MockReplaceDDL", `return(true)`))
	require.NoError(t, dom.Init(nil, nil))
	require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/pkg/ddl/NoDDLDispatchLoop"))
	require.NoError(t, failpoint.Disable("github.com/pingcap/tidb/pkg/domain/MockReplaceDDL"))

	// StartSchemaLoad should not panic when Init has been called
...
func TestReadOnlyDomainInfoSchema(t *testing.T) {
	s, err := mockstore.NewMockStore()
	require.NoError(t, err)
	dom := NewDomain(s, 1*time.Second, 0, 0, nil)
	defer dom.Close()
...
func TestDomainStartSchemaLoadBeforeStart(t *testing.T) {
	// Verify StartSchemaLoad can be called instead of Start().
	// This is the PCR read-only path.
	s, err := mockstore.NewMockStore()
	require.NoError(t, err)
	dom := NewDomain(s, 1*time.Second, 0, 0, nil)
	defer dom.Close()
	...
	// StartSchemaLoad skips DDL ownership; Start() would try to campaign
	dom.StartSchemaLoad()

	// DDL should exist but not be running as owner (no Start was called)
	require.NotNil(t, dom.DDL())
	time.Sleep(50 * time.Millisecond)
}
