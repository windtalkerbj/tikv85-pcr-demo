    ...
    ER: RaftEngine,
{
    ...
    fn init_servers<F: KvFormat>(&mut self) -> Arc<VersionTrack<ServerConfig>> {
        ...
        // Create SstImporter early so it can be injected into CDC Endpoint
        // for PCR IngestSst handling.
        ...
        // Inject SstImporter into CDC Endpoint for PCR IngestSst handling.
        // This allows CDC Delegates to read SST files from Lightning/fast-DDL
        // writes and stream the KV data to PCR Consumer clusters.
        ...
        // PCR (stream-ingest) is only used in the raft-v1 path (server.rs).
        // raftstore-v2 path (server2.rs) skips stream-ingest entirely.
    ...
    fn register_services(&mut self) {
        ...
        // Register PCR gRPC service (Producer-side).
        {
            info!("registering PCR PcrStream gRPC service (server2)");
            // Source-side PCR batcher defaults: 1MB buffer, 150ms flush.
            // Different from consumer-side (128MB/150ms) because source batches
            // per-Region CDC data which arrives in smaller chunks.
            ...
            let pcr_service = cdc::pcr_service::Service::new(
                ...
                None, // server2: pass engine from RaftKv2 when needed, None for now
