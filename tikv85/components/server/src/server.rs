...
use stream_ingest::{self, StreamIngestConfigManager};
...
struct Servers<RocksEngine: KvEngine, ER: RaftEngine, F: KvFormat> {
    ...
    stream_ingest_scheduler: Option<tikv_util::worker::Scheduler<stream_ingest::Task>>,
    ...
    F: KvFormat,
{
    ...
    fn init_servers(&mut self) -> Arc<VersionTrack<ServerConfig>> {
        ...
        // Start stream-ingest (PCR consumer) — requires tablet_registry (raftstore-v2 compatible)
            ...
            && self.engines.is_some()
        {
            ...
            let engines = self.engines.as_ref().unwrap();
            let kv_engine = Arc::new(engines.engines.kv.clone());
    let write_guard = storage.write_protection_guard();
            let pd_client = self.pd_client.clone();

            let task = stream_ingest::create_stream_ingest_task(
                ingest_cfg.enable,
                ingest_cfg.source_address.clone(),
                ingest_cfg.target_tidb_address.clone(),
                ingest_cfg.max_kv_buffer_size.0 as usize,
                ingest_cfg.max_range_key_buffer_size.0 as usize,
                ingest_cfg.checkpoint_interval.as_secs(),
                ingest_cfg.num_subscription_threads,
                kv_engine,
                pd_client.clone(),
                self.core.store_path.join("pcr"), write_guard,
            );

            let mut worker = Box::new(
                self.core
                    .background_worker
                    .lazy_build("stream-ingest"),
            );
            let scheduler = worker.scheduler();

            cfg_controller.register(
                tikv::config::Module::StreamIngest,
                Box::new(StreamIngestConfigManager::new(
                    scheduler.clone(),
                )),
            );

            // PCR: forward UnsafeDestroyRange (DROP TABLE / TRUNCATE) to consumer.
            let (dr_tx, dr_rx) = std::sync::mpsc::channel::<(Vec<u8>, Vec<u8>)>();
            tikv::server::service::set_pcr_delete_range_forwarder(dr_tx);
            let ingest_ctx_for_dr = task.ingest_ctx();
            std::thread::spawn(move || {
                while let Ok((start_key, end_key)) = dr_rx.recv() {
                    ingest_ctx_for_dr.apply_delete_range(&start_key, &end_key);
                }
            });

            // Start PCR HTTP control server for pcr-ctl communication
            let pcr_http_port: u16 = 20190;
            let pcr_http_server = Arc::new(stream_ingest::http_control::PcrControlServer::new(pcr_http_port));
            pcr_http_server.set_scheduler(scheduler.clone());
            pcr_http_server.clone().start();

            worker.start(task);
            self.core.to_stop.push(worker);
            Some(scheduler)
        } else {
            None
        };
        ...
        // Create SstImporter early — needed for PCR injection into CDC Endpoint
        ...
        if self.core.config.stream_ingest.enable {
            cdc_endpoint.set_sst_importer(importer.clone());
        }

        // PCR: shared resolved_ts between CDC endpoint and PCR bridge threads.
        let pcr_resolved_ts = Arc::new(std::sync::atomic::AtomicU64::new(0));
        cdc_endpoint.set_pcr_resolved_ts_guard(pcr_resolved_ts.clone());
    ...
    fn register_services(&mut self) {
        ...
        // Register PCR gRPC service (Producer-side — always registered)
        {
            info!("registering PCR PcrStream gRPC service");
            // Source-side PCR batcher defaults: 1MB buffer, 150ms flush.
            let event_buf = 1 * 1024 * 1024; // 1MB
            let event_flush_ms = 150; // 150ms

            let pcr_service = cdc::pcr_service::Service::new(
                servers.cdc_scheduler.clone(),
                Some(Arc::new(engines.engines.kv.clone())),
                servers.pcr_resolved_ts.clone(),
                event_buf,
                event_flush_ms,
            );
            if servers
                .server
                .register_service(cdc::pcr_service::create_pcr_stream_service(pcr_service))
                .is_some()
            {
                fatal!("failed to register PCR stream service");
            }
            info!("PCR PcrStream gRPC service registered successfully");
        }
