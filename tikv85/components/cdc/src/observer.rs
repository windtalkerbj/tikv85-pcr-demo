...
use tikv_util::{error, info, memory::MemoryQuota, warn, worker::Scheduler};
...
impl<E: KvEngine> CmdObserver<E> for CdcObserver {
    ...
    fn on_flush_applied_cmd_batch(
        ...
        let batch_count = cmd_batches.len();
        let batch_levels: Vec<_> = cmd_batches.iter().map(|cb| (cb.region_id, cb.level)).collect();
        info!("cdc observer: on_flush_applied_cmd_batch";
            "max_level" => ?max_level,
            "batch_count" => batch_count,
            "batch_levels" => ?batch_levels,
        );

        // PCR: also process LockRelated batches so that live transactional
        // writes (Prewrite) are captured. Without this, INSERT/UPDATE/DELETE
        // during live PCR are silently dropped at the observer level.
        let min_level = if max_level >= ObserveLevel::All {
            ObserveLevel::All
        } else if max_level >= ObserveLevel::LockRelated {
            ObserveLevel::LockRelated
        } else {
            info!("cdc observer: skipping — max_level < LockRelated";
                "max_level" => ?max_level);
            return;
        };
        let cmd_batches: Vec<_> = cmd_batches
            .iter()
            .filter(|cb| cb.level >= min_level && !cb.is_empty())
            .cloned()
            .collect();
        if cmd_batches.is_empty() {
            info!("cdc observer: skipping — no batches after filter";
                "min_level" => ?min_level);
            return;
        }
        let region_ids: Vec<u64> = cmd_batches.iter().map(|cb| cb.region_id).collect();
        info!("cdc observer: SCHEDULING MultiBatch"; "regions" => ?region_ids);
