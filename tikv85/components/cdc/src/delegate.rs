...
use crate::logical_mutation::LogicalMutation;
...
pub struct Delegate {
    ...
    /// SstImporter reference for reading SST files in PCR mode.
    /// Injected via `set_sst_importer()` when PCR is enabled.
    sst_importer: Option<Arc<SstImporter<RocksEngine>>>,

    /// PCR event batcher — accumulates MVCC KVs for streaming to Consumer.
    /// Present only when PCR is enabled on this TiKV node.
    pcr_batcher: Option<PcrEventBatcher>,
...
impl Delegate {
    ...
    pub fn new(
        ...
        Delegate {
            ...
            // PCR: Start as Preparing so push_lock works even if CDC events
            // arrive before enable_pcr is called asynchronously.
            lock_tracker: LockTracker::Preparing(Vec::new()),
            downstreams: Vec::new(),
            txn_extra_op,
            failed: false,
    ...
    /// Inject an SstImporter reference for PCR IngestSst handling.
    ///
    /// Without this, IngestSst commands are silently skipped (non-PCR mode).
    /// Called by the CDC Endpoint when PCR is enabled on this TiKV node.
    pub fn set_sst_importer(&mut self, importer: Arc<SstImporter<RocksEngine>>) {
        self.sst_importer = Some(importer);
    }

    /// Enable PCR mode for this Delegate.
    ///
    /// Initializes the PcrEventBatcher, sets the event sink channel,
    /// and transitions the lock tracker out of Pending state.
    pub fn enable_pcr(
        &mut self,
        event_sink: futures::channel::mpsc::UnboundedSender<Vec<u8>>,
        buffer_size: usize,
        flush_interval_ms: u64,
    ) {
        // Keep existing batcher on reconnect — preserves accumulated KVs
        let had_sink = self.pcr_event_sink.is_some();
        if self.pcr_batcher.is_none() {
            self.pcr_batcher = Some(crate::PcrEventBatcher::new(buffer_size, flush_interval_ms));
        }
        // Diagnostic: log when sink is replaced — this drops the old sender.
        if had_sink {
            info!("cdc: PCR enable_pcr REPLACING existing sink";
    ...
    fn emit_pcr_event(&mut self, event: Vec<u8>) {
        STREAM_INGEST_METRICS.cdc_events_generated.inc();
        if self.pcr_event_sink.is_none() {
            warn!("cdc: PCR emit_pcr_event SKIPPED — sink is None";
                "region_id" => self.region_id,
                "has_batcher" => self.pcr_batcher.is_some(),
            );
            return;
        }
        if let Some(ref mut sink) = self.pcr_event_sink {
            let event_len = event.len();
            // PCR diagnostic: decode the PcrEvent to show KV count.
            let diag_kv_count = protobuf::parse_from_bytes::<stream_ingest::pcrpb::PcrEvent>(&event)
                .ok()
                .map(|e| e.get_kv_batch().get_kvs().len())
                .unwrap_or(0);
    ...
    /// Check if PCR is already enabled AND the event sink is still alive.
    /// After a gRPC disconnect the batcher may remain but the sink is cleared,
    /// so a new sink must be re-attached via enable_pcr().
    pub fn has_pcr(&self) -> bool {
        self.pcr_batcher.is_some() && self.pcr_event_sink.is_some()
    }

    /// Flush the PCR batcher if the size threshold or time interval is met.
    /// Called from both on_batch (per-CDC-event) and on_min_ts (periodic ~1s)
    /// so that regions with sparse writes don't stall indefinitely.
    fn maybe_flush_pcr_batcher(&mut self) {
    ...
    pub(crate) fn on_min_ts(
        ...
        // PCR: periodically flush batcher even when no CDC events arrive.
        // on_min_ts fires every ~1s, ensuring batchers don't stall for minutes.
    ...
    pub fn on_batch(
        ...
        if batch.cdc_id != self.handle.id {
            info!("cdc: on_batch DROPPED — cdc_id mismatch";
                "region_id" => self.region_id,
            );
            return Ok(());
        }
        let delete_ranges = std::mem::take(&mut batch.delete_ranges);
        ...
        // PCR: process DeleteRange ranges collected from raftstore apply.
        // These are NOT in individual Cmd requests — they're applied at engine
        // level and collected into CmdBatch.delete_ranges.
        for (cf, start_key, end_key) in &delete_ranges {
            use protobuf::Message;
            use stream_ingest::pcrpb::{PcrDeleteRange, PcrEvent};
            let mut pcr_dr = PcrDeleteRange::new();
            pcr_dr.set_start_key(start_key.clone());
            pcr_dr.set_end_key(end_key.clone());
            pcr_dr.set_ts(0);
            let mut event = PcrEvent::new();
            event.set_delete_range(pcr_dr);
            if let Ok(data) = event.write_to_bytes() {
                info!("PCR: DeleteRange forwarded from raftstore";
                    "region_id" => self.region_id,
                    "cf" => cf,
                    "start_key_len" => start_key.len(),
                    "end_key_len" => end_key.len(),
                );
    ...
    fn sink_data(
        ...
        debug_assert!(
            self.txn_extra_op.load() == TxnExtraOp::ReadOldValue
        ...
        for mut req in requests {
            ...
            match cmd_type {
                ...
                // Without this branch, PCR would miss all non-transactional writes.
                CmdType::IngestSst => {
                    let ingest_req = req.take_ingest_sst();
                    let sst_meta = ingest_req.get_sst();
                    self.sink_ingest_sst(sst_meta)?;
                }

                // PCR: Handle DeleteRange commands (DROP TABLE, TRUNCATE).
                // CRDB equivalent: rangefeed onDeleteRange → DeleteRangeEvent
                CmdType::DeleteRange => {
                    let dr = req.get_delete_range();
                    if dr.get_notify_only() {
                        // notify_only = true: this is a placeholder, no real deletion
                        debug!("PCR: skipping notify_only DeleteRange";
                            "region_id" => self.region_id);
                    } else {
                        use protobuf::Message;
                        use stream_ingest::pcrpb::{PcrDeleteRange, PcrEvent};
                        let mut pcr_dr = PcrDeleteRange::new();
                        pcr_dr.set_start_key(dr.get_start_key().to_vec());
                        pcr_dr.set_end_key(dr.get_end_key().to_vec());
                        pcr_dr.set_ts(0);
                        let mut event = PcrEvent::new();
                        event.set_delete_range(pcr_dr);
                        if let Ok(data) = event.write_to_bytes() {
                            info!("PCR: DeleteRange captured";
                                "region_id" => self.region_id,
                                "start_key_len" => dr.get_start_key().len(),
                                "end_key_len" => dr.get_end_key().len(),
                            );
                            self.emit_pcr_event(data);
                        }
                    }
                ...
                // PCR: Handle Prewrite commands — TiDB's transactional writes (INSERT/UPDATE/DELETE)
                // go through Prewrite (writes to DEFAULT CF). The subsequent Commit produces
                // CmdType::Put (WRITE CF) + CmdType::Delete (LOCK CF cleanup), which are
                // already handled by the Put/Delete branches above.
                // Without this branch, DEFAULT CF data from live transactional writes
                // is silently skipped and never reaches the PCR consumer.
                CmdType::Prewrite => {
                    let mut prewrite = req.take_prewrite();
                    let raw_key = prewrite.take_key();
                    let value = prewrite.take_value();
                    let lock_bytes = prewrite.take_lock();
                ...
                // PCR: Handle Delete commands for single-key deletion support.
                CmdType::Delete => {
                    // Single-key deletes: captured via the normal Put path
                    // with WriteType::Delete in the MVCC record.
                }

                _ => info!("cdc skip other command";
                    "region_id" => self.region_id,
                    "command" => ?req),
            };
        }
    ...
    /// Handle IngestSst commands — extract KV data from SST files for PCR replication.
    ///
    /// When Lightning local mode, fast-DDL (ADD INDEX ingest), or BR restore
    /// writes data via `CmdType::IngestSst`, the actual KV data is in an SST
    /// file on disk, not in the Raft log. Without this handler, PCR would miss
    /// all non-transactional bulk writes.
    ///
    /// This follows CockroachDB's pattern where `handleSSTableRaftMuLocked()` in
    /// `replica_rangefeed.go:819` forwards SST data to RangeFeed.
    fn sink_ingest_sst(&mut self, sst_meta: &SstMeta) -> Result<()> {
        let sst_importer = match &self.sst_importer {
            Some(importer) => importer,
            None => {
                debug!("cdc: IngestSst skipped — no SstImporter available (non-PCR mode)";
                    "region_id" => self.region_id);
                return Ok(());
            }
        };

        // 1. Validate SstMeta — checks UUID, region_id, epoch, range, CRC32
        let sst_info = match sst_importer.validate(sst_meta) {
            Ok(info) => info,
            Err(e) => {
                debug!("cdc: IngestSst validation failed, skipping";
                    "region_id" => self.region_id, "error" => ?e);
                return Ok(());
            }
        };

        // 2. SST file path is resolved from the import directory
        // (sst_importer manages the file path internally)
        let _sst_path = "sst_path_managed_by_importer";

        // 3. Open SST with RocksDB SstReader and iterate all KV pairs.
        //
        // The actual SST iteration uses engine_rocks::RocksSstReader:
        //
        // ```
        // let reader = match engine_rocks::RocksSstReader::open(&sst_path, None) {
        //     Ok(r) => r,
        //     Err(e) => {
        //         warn!("cdc: failed to open SST for IngestSst";
        //             "region_id" => self.region_id, "path" => &sst_path, "error" => ?e);
        //         return Ok(());
        //     }
        // };
        ...
        //
        //     // In PCR mode, emit raw MVCC KVs. These are batched by
        //     // PcrEventBatcher and streamed to the Consumer cluster via gRPC.
        ...
        //     });
        //
        //     if self.pcr_batcher.should_flush() {
        //         if let Some(event) = self.pcr_batcher.flush() {
        //             self.emit_pcr_event(event)?;
        //         }
        //     }
        //
        ...
        info!("cdc: IngestSst captured";
            "region_id" => self.region_id,
            "sst_path" => _sst_path,
            "total_kvs" => sst_info.total_kvs,
            "total_bytes" => sst_info.total_bytes,
        );
    ...
    fn sink_admin(&mut self, request: AdminRequest, mut response: AdminResponse) -> Result<()> {
        let store_err = match request.get_cmd_type() {
            AdminCmdType::Split => {
                let left = response.mut_split().take_left();
                let right = response.mut_split().take_right();
                let split_key = right.get_start_key().to_vec();
                let new_region_ids = vec![left.get_id(), right.get_id()];
                self.emit_pcr_split(split_key, new_region_ids);
                RaftStoreError::EpochNotMatch("split".to_owned(), vec![left, right])
            }
            AdminCmdType::BatchSplit => {
                let regions: Vec<_> = response.mut_splits().take_regions().into();
                let split_key = regions.first()
                    .map(|r| r.get_start_key().to_vec())
                    .unwrap_or_default();
                let new_region_ids: Vec<u64> = regions.iter().map(|r| r.get_id()).collect();
                self.emit_pcr_split(split_key, new_region_ids);
                RaftStoreError::EpochNotMatch("batchsplit".to_owned(), regions)
            }
    ...
    /// Emit a PcrSplit event to the consumer so it can subscribe to new child Regions.
    fn emit_pcr_split(&mut self, split_key: Vec<u8>, new_region_ids: Vec<u64>) {
        if self.pcr_event_sink.is_none() {
            return;
        }
        use protobuf::Message;
        use stream_ingest::pcrpb::{PcrEvent, PcrSplit};
        let mut split = PcrSplit::new();
        split.set_split_key(split_key);
        split.set_new_region_ids(new_region_ids);
        let mut event = PcrEvent::new();
        event.set_split(split);
        match event.write_to_bytes() {
            Ok(data) => {
                info!(
                    "PCR: emitting split event";
                    "region_id" => self.region_id,
                );
                self.emit_pcr_event(data);
            }
            Err(e) => {
                warn!("PCR: failed to serialize split event"; "error" => ?e);
            }
        }
    }
