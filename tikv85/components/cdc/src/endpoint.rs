...
use raftstore::{
    ...
    router::CdcHandle,
    store::{
        fsm::{apply::ChangeObserver, store::StoreRegionMeta},
...
pub enum Task {
    ...
    /// Start PCR event streaming for a specific Region.
    /// The CDC endpoint captures KV changes for this Region and sends
    /// `PcrEvent` messages through `event_sink` to the gRPC stream.
    StartPcrStream {
        region_id: u64,
        start_ts: u64,
        /// Channel to send serialized PCR events to the gRPC service
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            Task::ChangeConfig(change) => de
                ...
                .field("change", change)
                .finish(),
            Task::StartPcrStream { region_id, .. } => de
                .field("type", &"start_pcr_stream")
                .field("region_id", &region_id)
                .finish(),
...
pub struct Endpoint<T, E, S> {
    ...
    /// SstImporter for PCR IngestSst handling.
    /// Set when PCR is enabled on this TiKV node.
    ...
    /// PCR: shared resolved_ts updated on each on_min_ts cycle.
    /// The PCR bridge thread reads this to generate checkpoints with a
    /// real PD TSO instead of wall clock.
    pcr_resolved_ts: Arc<AtomicU64>,

    /// PCR: span-level subscription registry. Holds shared sinks that are
    /// auto-attached to delegates when they're created (including after split).
...
impl<T: 'static + CdcHandle<E>, E: KvEngine, S: StoreRegionMeta> Endpoint<T, E, S> {
    pub fn new(
        ...
        let ep = Endpoint {
            ...
            pcr_resolved_ts: Arc::new(AtomicU64::new(0)),
    ...
    /// Set the PCR resolved_ts guard so the bridge thread can read real CDC timestamps.
    pub fn set_pcr_resolved_ts_guard(&mut self, guard: Arc<AtomicU64>) {
        self.pcr_resolved_ts = guard;
    }
    ...
    /// Inject an SstImporter for PCR IngestSst handling.
    ///
    /// When PCR is enabled, the server passes the SstImporter to the CDC
    /// Endpoint so that newly created Delegates can read SST file contents
    /// for replication. Without this, IngestSst commands are skipped.
    pub fn set_sst_importer(&mut self, importer: Arc<SstImporter<RocksEngine>>) {
        self.sst_importer = Some(importer);
    }

    /// Start PCR event streaming for a Region.
    /// Auto-registers the Region for CDC observation if not already tracked.
    fn on_start_pcr_stream(
        &mut self,
        region_id: u64,
        _start_ts: u64,
        event_sink: futures::channel::mpsc::UnboundedSender<Vec<u8>>,
        ...
        info!("cdc: starting PCR stream"; "region_id" => region_id);

        // If Delegate doesn't exist yet, auto-create one.
        // PCR subscriptions arrive without a prior CDC Downstream registration,
        // so we must set up CDC observation for the Region ourselves.
        ...
        let delegate = match self.capture_regions.entry(region_id) {
            ...
            HashMapEntry::Vacant(e) => {
                ...
                let txn_extra_op = match self.store_meta.lock().unwrap().reader(region_id) {
                    ...
                    None => {
                        warn!("cdc: PCR stream for unknown region"; "region_id" => region_id);
                        let _ = event_sink;
                        return;
                    }
                };
                ...
                if let Some(ref importer) = self.sst_importer {
                    delegate.set_sst_importer(importer.clone());
                }
                let observe_id = delegate.handle.id;
                self.observer.subscribe_region(region_id, observe_id);
                info!("cdc: auto-created delegate for PCR stream";
                    "region_id" => region_id, "observe_id" => ?observe_id);
                e.insert(delegate)
            }
        };

        // Upgrade the Region's ObserveLevel to All so the CDC observer
        // receives ALL Raft-apply events including DEFAULT CF writes.
        // PCR needs ObserveLevel::All; without capture_change, existing
        // delegates (created by RTS at startup) stay at LockRelated level
        // and DEFAULT CF writes from Prewrites are silently dropped.
        //
        // Use fresh_handle to create an independent observing flag.
        // Cloning delegate.handle shares the same Arc<AtomicBool>, so when
        // the delegate is later deregistered, the raftstore's observe_info
        // also gets stopped — thus max_level < All.
        let observe_handle = delegate.handle.fresh_handle();
        // Look up the Region via PD (synchronous, cached).
        // For raftstore-v1 on localhost, this is an in-memory lookup.
        let (region_epoch, r_start, r_end) = match futures::executor::block_on(
            self.pd_client.get_region_by_id(region_id),
        ) {
            Ok(Some(region)) => (
            ...
            _ => {
                // Fallback: use default epoch — the raftstore will callback
                // with an error if it doesn't match, which we ignore.
                let mut epoch = kvproto::metapb::RegionEpoch::default();
        ...
        // PCR auto-match with actual region key range.
        Self::auto_match_pcr(delegate, &self.pcr_registry, &r_start, &r_end);

        let change_observer = ChangeObserver::from_cdc(region_id, observe_handle);
        let cdc_handle = self.cdc_handle.clone();
        if let Err(e) = cdc_handle.capture_change(
            region_id,
            region_epoch,
            change_observer,
            Callback::read(Box::new(move |resp: ReadResponse<_>| {
                if resp.response.get_header().has_error() {
                    let err = resp.response.get_header().get_error();
                    warn!("cdc: PCR capture_change failed";
                        "region_id" => region_id,
                        "error" => ?err);
                } else {
                    info!("cdc: PCR capture_change acknowledged";
                        "region_id" => region_id);
                }
            })),
        ) {
            warn!("cdc: PCR capture_change failed"; "region_id" => region_id, "error" => ?e);
        } else {
            info!("cdc: PCR capture_change sent"; "region_id" => region_id);
        }

        // PCR sink is set up by auto_match_pcr above (from PcrRegistry).
        // Only call enable_pcr here if the delegate still has no sink
        // (backward compat with old StartPcrStream without PcrRegistry).
        if !delegate.has_pcr() {
            delegate.enable_pcr(event_sink, event_buffer_size, event_flush_interval_ms);
            info!("cdc: PCR stream attached"; "region_id" => region_id);
        }
    }
    ...
    /// Auto-attach PCR sink to a delegate if a span subscription is active.
    /// Uses empty keys for matching — all active spans are full-key-range.
    fn auto_match_pcr(delegate: &mut Delegate, registry: &PcrRegistry, r_start: &[u8], r_end: &[u8]) {
        if let Some((sink, buf_size, flush_ms)) =
            registry.match_region(0, r_start, r_end)
        {
            delegate.enable_pcr((*sink).clone(), buf_size, flush_ms);
        }
    }
    ...
    pub fn on_register(&mut self, mut request: ChangeDataRequest, mut downstream: Downstream) {
        ...
        let delegate = match self.capture_regions.entry(region_id) {
            ...
            HashMapEntry::Vacant(e) => {
                ...
                // Inject SstImporter for PCR IngestSst handling
                if let Some(ref importer) = self.sst_importer {
                    delegate.set_sst_importer(importer.clone());
                }
                e.insert(delegate)
        ...
        // PCR auto-match: look up region key range from PD, then
        // check if a span subscription covers this region.
        let (r_start, r_end) = futures::executor::block_on(
            self.pd_client.get_region_by_id(region_id),
        ).ok().flatten()
            .map(|r| (r.get_start_key().to_vec(), r.get_end_key().to_vec()))
            .unwrap_or_default();
        Self::auto_match_pcr(delegate, &self.pcr_registry, &r_start, &r_end);
        let observe_id = delegate.handle.id;
        ...
        for batch in multi {
            ...
            if let Some(delegate) = self.capture_regions.get_mut(&region_id) {
                if delegate.has_failed() {
                    info!("cdc: on_multi_batch skipping failed delegate"; "region_id" => region_id);
                    continue;
                }
    ...
    fn on_min_ts(&mut self, regions: Vec<u64>, min_ts: TimeStamp, current_ts: TimeStamp) {
        ...
        // Update PCR shared resolved_ts for checkpoint generation.
        // Use current_ts (PD TSO) so the bridge thread has a real MVCC timestamp.
        self.pcr_resolved_ts
            .store(self.current_ts.into_inner(), Ordering::Release);
    }
    ...
    for Endpoint<T, E, S>
{
    ...
    fn run(&mut self, task: Task) {
        ...
        match task {
            ...
            // PCR: Register a span-level subscription (span-native path).
            // SpanBridge calls this once for the full key range. Delegates
            // auto-attach via PcrRegistry on birth.
            Task::RegisterSpanSubscription {
                ...
                let has_sub = self.pcr_registry.match_region(0, &[], &[]).is_some();
            ...
            // PCR: Start streaming events for a specific Region (DEPRECATED).
            // SpanBridge now uses RegisterSpanSubscription. This is kept for
            // backward compat with old SpanBridge binary.
            Task::StartPcrStream {
                region_id,
                start_ts,
                event_sink,
                event_buffer_size,
                event_flush_interval_ms,
            } => self.on_start_pcr_stream(
                region_id, start_ts, event_sink,
                event_buffer_size, event_flush_interval_ms,
            ),
        }
    }
}
