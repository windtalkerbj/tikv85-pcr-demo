...
impl ObserveHandle {
    ...
    /// Create a new handle with the same ID but a FRESH observing flag.
    /// Unlike clone(), this does not share the observing Arc, so stopping
    /// the original handle does not affect the new one. Used by PCR to
    /// decouple delegate lifecycle from raftstore observe_info.
    pub fn fresh_handle(&self) -> ObserveHandle {
        ObserveHandle::with_id(self.id.0)
    }
...
pub struct CmdBatch {
    ...
    pub region_id: u64,
    pub cmds: Vec<Cmd>,
    /// DeleteRange ranges collected during apply (DROP TABLE / TRUNCATE).
    /// Each entry: (cf, start_key, end_key).
    pub delete_ranges: Vec<(String, Vec<u8>, Vec<u8>)>,
}
...
/// PCR: channel for forwarding UnsafeDestroyRange (DROP TABLE / TRUNCATE) from
/// source TiKV's gRPC handler to the CDC PcrService, which broadcasts to all
/// active PCR subscribers.
pub static PCR_DELETE_RANGE_FORWARDER: std::sync::Mutex<
    Option<std::sync::mpsc::Sender<(Vec<u8>, Vec<u8>)>>,
> = std::sync::Mutex::new(None);

/// Register the PCR UnsafeDestroyRange forwarder. Called from CDC PcrService
/// on the source TiKV, or from server.rs on the target TiKV.
