...
pub struct Config {
    ...
    #[online_config(skip)]
    pub prevote: bool,
    /// Number of regions to pre-split the initial region into during bootstrap.
    /// Only effective in raftstore-v2. Default: 64.
    /// Pre-creates per-Region RocksDB tablets to reduce TiDB bootstrap latency.
    #[online_config(skip)]
    pub bootstrap_pre_split_count: usize,
    #[online_config(skip)]
