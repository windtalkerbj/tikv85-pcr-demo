...
impl<S: Snapshot> ApplyCallbackBatch<S> {
    ...
    fn last_cmd_batch_mut(&mut self) -> Option<&mut CmdBatch> {
        self.cmd_batch.last_mut()
    }
    ...
    EK: KvEngine,
{
    ...
    pub fn write_to_db(&mut self) -> (bool, Option<SequenceNumber>) {
        ...
        // PCR diagnostic: log which regions are flushed and their observe levels.
        let diag_regions: Vec<(u64, ObserveLevel)> = cmd_batch
            .iter()
            .map(|b| (b.region_id, b.level))
            .collect();
        self.host
    ...
    fn apply_raft_cmd(
        ...
        if let ApplyResult::Res(ref exec_result) = exec_result {
            match *exec_result {
                ...
                ExecResult::DeleteRange { ref ranges } => {
                    // PCR: forward DeleteRange ranges to CDC observer via CmdBatch.
                    if let Some(batch) = ctx.applied_batch.last_cmd_batch_mut() {
                        for r in ranges {
                            batch.delete_ranges.push((
                                r.cf.to_owned(),
                                r.start_key.to_vec(),
                                r.end_key.to_vec(),
                            ));
