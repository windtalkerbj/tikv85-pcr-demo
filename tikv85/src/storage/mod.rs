...
pub struct Storage<E: Engine, L: LockManager, F: KvFormat> {
    ...
    /// PCR: when true, raw writes are rejected (target cluster in replication mode).
    write_protected: Arc<AtomicBool>,
...
impl<E: Engine, L: LockManager, F: KvFormat> Storage<E, L, F> {
    ...
    pub fn from_engine<R: FlowStatsReporter>(
        ...
        Ok(Storage {
            ...
            quota_limiter,
            resource_manager,
            write_protected: Arc::new(AtomicBool::new(false)),
            _phantom: PhantomData,
        })
    }

    /// Enable/disable PCR write protection.
    /// When true, raw writes are rejected (target cluster in replication mode).
    pub fn set_write_protection(&self, protected: bool) {
        self.write_protected.store(protected, Ordering::Release);
    }
    ...
    /// Get the write protection guard so PCR can control it.
    pub fn write_protection_guard(&self) -> Arc<AtomicBool> {
        self.write_protected.clone()
    }
    ...
    pub fn sched_txn_command<T: StorageCallbackType>(
        ...
        // PCR: reject user transactional writes while replication is in progress.
        // TiDB tags user statements with request_source (e.g. "Insert", "Delete").
        // Internal operations (DDL owner, auto-analyze, bootstrap) have empty
        // request_source and are allowed — they're needed for cluster liveness.
        // PCR SST ingest bypasses this path entirely (DirectIngest).
        match &cmd {
            Command::Prewrite(_) | Command::PrewritePessimistic(_)
            | Command::AcquirePessimisticLock(_) | Command::AcquirePessimisticLockResumed(_)
            | Command::Flush(_) => {
                let source = cmd.ctx().get_request_source();
                // User writes have request_source like "Insert"/"Delete"/"Update".
                // Internal writes use "internal_*" prefix (e.g. "internal_gc").
                // Empty request_source also indicates internal operation.
                if self.write_protected() && !source.is_empty() && !source.starts_with("internal_") {
                    callback(Err(Error::from(ErrorInner::Other(box_err!(
                        "target cluster is in PCR replication mode; writes not allowed until activation"
                    )))));
                    return Ok(());
                }
            }
    ...
    pub fn raw_put(
        ...
        if self.write_protected() {
            callback(Err(box_err!("target cluster is in PCR replication mode; writes not allowed until activation")));
            return Ok(());
        }
        let api_version = self.api_version;
