...
/// PCR: channel for forwarding UnsafeDestroyRange calls to the stream-ingest consumer.
use std::sync::Mutex;
pub static PCR_DELETE_RANGE_FORWARDER: Mutex<Option<std::sync::mpsc::Sender<(Vec<u8>, Vec<u8>)>>> =
    Mutex::new(None);

pub fn set_pcr_delete_range_forwarder(tx: std::sync::mpsc::Sender<(Vec<u8>, Vec<u8>)>) {
    *PCR_DELETE_RANGE_FORWARDER.lock().unwrap() = Some(tx);
}

/// PCR: forward a DeleteRange to the registered PCR consumer forwarder.
/// Returns true if a forwarder was registered and the range was sent.
pub fn forward_delete_range(start_key: Vec<u8>, end_key: Vec<u8>) -> bool {
    if let Some(tx) = PCR_DELETE_RANGE_FORWARDER.lock().unwrap().as_ref() {
...
impl<E: Engine, L: LockManager, F: KvFormat> Tikv for Service<E, L, F> {
    ...
    fn unsafe_destroy_range(
        ...
        // PCR: forward DeleteRange to consumer.
        // UnsafeDestroyRange bypasses Raft, so the CDC observer never sees it.
        // The forwarder is only registered when PCR is active on this node.
        // write_protected() is always false on the source TiKV, so don't gate on it.
        let _ = forward_delete_range(
...
mod tests {
    ...
    // Lock to serialize tests that mutate the global PCR_DELETE_RANGE_FORWARDER.
    static DR_TEST_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

    #[test]
    fn test_forward_delete_range_without_forwarder() {
        let _guard = DR_TEST_LOCK.lock().unwrap();
        // Ensure no forwarder is registered
        *PCR_DELETE_RANGE_FORWARDER.lock().unwrap() = None;
        let result = forward_delete_range(
            b"t_100_".to_vec(),
            b"t_101_".to_vec(),
        );
        assert!(!result, "should return false when no forwarder is registered");
    }

    #[test]
    fn test_forward_delete_range_with_forwarder() {
        let _guard = DR_TEST_LOCK.lock().unwrap();
        let (tx, rx) = std::sync::mpsc::channel::<(Vec<u8>, Vec<u8>)>();
        set_pcr_delete_range_forwarder(tx);

        let start_key = b"t_100_".to_vec();
        let end_key = b"t_101_".to_vec();
        let result = forward_delete_range(
            start_key.clone(),
            end_key.clone(),
        );
        assert!(result, "should return true when forwarder is registered");

        let (got_start, got_end) = rx.recv_timeout(std::time::Duration::from_secs(1))
            .expect("should receive forwarded delete range");
        assert_eq!(got_start, start_key);
        assert_eq!(got_end, end_key);
