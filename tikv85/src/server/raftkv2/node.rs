    ...
    ER: RaftEngine,
{
        ...
        T: Transport + 'static,
    {
        ...
        if let Some(region) = bootstrap.bootstrap_first_region(&self.store, store_id)? {
            ...
            // Pre-split the initial region to speed up TiDB bootstrap.
            // raftstore-v2 creates per-Region RocksDB tablets; doing this
            // upfront avoids costly sequential splits during TiDB init.
            let pre_split_count = store_cfg.value().bootstrap_pre_split_count;
            if pre_split_count > 1 {
                bootstrap.set_pre_split_count(pre_split_count);
                if let Ok(pre_regions) = bootstrap.pre_split_initial_region(store_id, &region) {
                    info!(self.logger, "bootstrap pre-split completed"; "region_count" => pre_regions.len(), "pre_split_count" => pre_split_count);
                    // Open tablet for each pre-split region
                    for r in &pre_regions {
                        let rpath = registry.tablet_path(r.get_id(), RAFT_INIT_LOG_INDEX);
                        let rctx = TabletContext::new(r, Some(RAFT_INIT_LOG_INDEX));
                        if let Err(e) = registry.tablet_factory().open_tablet(rctx, &rpath) {
                            warn!(self.logger, "failed to open pre-split tablet"; "region_id" => r.get_id(), "error" => ?e);
                        }
                    }
                }
            }
