...
impl<ER: RaftEngine> DebuggerImplV2<ER> {
    ...
    /// Scan raw KV pairs across all tablets in the specified key range.
    /// Discovers tablets from the filesystem (no raft engine needed),

    /// enabling scans while TiKV is running.
    pub fn raw_scan(
        &self,
        start: &[u8],
        end: &[u8],
        ...
        use engine_traits::{IterOptions, Iterable, Iterator as EngineIterator};
        ...
        // Discover tablets from filesystem (no raft engine required).
        // Each tablet directory is named <region_id>_<suffix>.
        if let Ok(entries) = std::fs::read_dir(tablet_root) {
            for entry in entries.flatten() {
                ...
                let Ok(region_id) = parts[0].parse::<u64>() else {
                    continue;
                };
                let suffix: u64 = parts[1].parse().unwrap_or(0);

                // Load tablet from registry (opens RocksDB if not cached)
                let ctx = TabletContext::with_infinite_region(region_id, Some(suffix));
                let mut tablet_cache = match self.tablet_reg.load(ctx, true) {
                    Ok(t) => t,
                    Err(_) => continue,
                };
                let tablet = match tablet_cache.latest() {
                    Some(t) => t,
                    None => continue,
                };

                // Build iterator bounds
                let iter_start = KeyBuilder::from_vec(start.to_vec(), 0, 0);
                let iter_end = if !end.is_empty() {
                    Some(KeyBuilder::from_vec(end.to_vec(), 0, 0))
                } else {
                    None
                };
                let iter_opt = IterOptions::new(Some(iter_start), iter_end, false);

                let mut iter = match tablet.iterator_opt(cf, iter_opt) {
                    Ok(it) => it,
                    Err(_) => continue,
                };

                if iter.seek_to_first().unwrap_or(false) {
                    results.push((iter.key().to_vec(), iter.value().to_vec()));
                    while results.len() < limit && iter.next().unwrap_or(false) {
                        results.push((iter.key().to_vec(), iter.value().to_vec()));
                    }
                }
            }
