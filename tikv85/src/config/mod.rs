...
pub struct StreamIngestConfig {
    /// Enable PCR consumer task on this TiKV node
    #[online_config(skip)]
    pub enable: bool,
    ...
    /// Maximum range-key buffer size before triggering flush
    pub max_range_key_buffer_size: ReadableSize,

    /// Minimum interval between flushes
    pub min_flush_interval: ReadableDuration,

    /// Source cluster gRPC address
    #[online_config(skip)]
    pub source_address: String,

    /// Target TiDB address for DDL monitoring (Span Control-Plane)
    #[online_config(skip)]
    ...
    /// Number of concurrent subscription threads
    #[online_config(skip)]
...
impl StreamIngestConfig {
    pub fn validate(&self) -> Result<(), Box<dyn Error>> {
        if self.enable && self.source_address.is_empty() {
            return Err("stream_ingest.source_address must be set when enable=true".into());
        }
        if self.max_kv_buffer_size.0 == 0 {
            return Err("stream_ingest.max_kv_buffer_size cannot be zero".into());
        }
        if self.min_flush_interval.is_zero() {
            return Err("stream_ingest.min_flush_interval cannot be zero".into());
        }
        Ok(())
    }
}
...
impl Default for TikvConfig {
    ...
    fn default() -> TikvConfig {
        TikvConfig {
            ...
            stream_ingest: StreamIngestConfig::default(),
