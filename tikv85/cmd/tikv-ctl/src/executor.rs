...
use std::{
    borrow::ToOwned, cmp::Ordering, path::{Path, PathBuf}, result, str,
    string::ToString, sync::Arc, time::Duration,
};

use api_version::{ApiV1, KvFormat};
use encryption_export::data_key_manager_from_config;
use engine_rocks::{util::{db_exist, new_engine_opt}, RocksEngine};
use engine_traits::{
    Engines, Error as EngineError, RaftEngine, TabletContext, TabletRegistry,
...
/// Try to hex-decode an ASCII hex string (e.g. "7480AABB" → [0x74, 0x80, ...]).
fn try_decode_hex(data: &[u8]) -> Option<Vec<u8>> {
    if data.len() % 2 != 0 { return None; }
    if !data.iter().all(|b| b.is_ascii_hexdigit()) { return None; }
...
pub fn new_debug_executor(
    ...
    if !cfg.raft_engine.enable {
        ...
        match engine_type {
            EngineType::RaftKv => {
                // Try to open the KV RocksDB normally. If TiKV is running,
                // the LOCK file prevents this — fall back to secondary instance.
                let kv_db = match factory.create_shared_db(data_dir) {
                    Ok(db) => db,
                    Err(e) => {
                        let kv_path = Path::new(data_dir).join("db");
                        if kv_path.exists() {
                            println!(
                                "KV DB locked (TiKV running): {}. Using secondary instance.",
                                e
                            );
                            let scanner = SingleDbScanner::new(kv_path);
                            return Box::new(scanner) as Box<_>;
                        }
                        handle_engine_error(e);
                    }
                };
                let raft_db =
                    RaftLogEngine::new(config, key_manager, None /* io_rate_limiter */).unwrap();
            ...
            EngineType::RaftKv2 => {
                ...
                // Use tablet-only scanner for RaftKv2. It handles raw_scan
                // directly from tablet RocksDB files, enabling scans while
                // TiKV is running (no raft engine or tablet lock needed).
                let scanner = TabletDebugExecutor::new(registry);
                Box::new(scanner) as Box<_>
            }
        }
    }
}
...
pub trait DebugExecutor {
    ...
    fn raw_scan(&self, from_key: &[u8], to_key: &[u8], limit: usize, cf: &str) {
        ...
        // Auto-detect hex-encoded keys (e.g. "--from 7480AABB").
        // The CLI uses unescape() which passes ASCII hex as raw bytes,
        // so we hex-decode if the input looks like a hex string.
        let from_bytes = try_decode_hex(from_key)
            .unwrap_or_else(|| from_key.to_vec());
        let to_bytes = try_decode_hex(to_key)
            .unwrap_or_else(|| to_key.to_vec());

        if !to_bytes.is_empty() && from_bytes.as_slice() >= to_bytes.as_slice() {
            eprintln!(
        ...
        self.raw_scan_impl(&from_bytes, &to_bytes, limit, cf);
    }
    ...
    /// Like raw_scan but computes SHA256 hash of all KV pairs instead of printing.
    fn raw_scan_hash(&self, from_key: &[u8], to_key: &[u8], limit: usize, cf: &str) {
        use sha2::{Digest, Sha256};
        let mut hasher = Sha256::new();
        self.raw_scan_impl_with_hash(from_key, to_key, limit, cf, Some(&mut hasher));
        println!("{}", hex::encode(hasher.finalize()));
    }

    /// Like raw_scan_impl but returns SHA256 hash if `hash` is Some.
    fn raw_scan_impl_with_hash(
        &self,
        from_key: &[u8],
        end_key: &[u8],
        limit: usize,
        cf: &str,
        mut hash: Option<&mut sha2::Sha256>,
    ) {
        // Default: do a raw_scan and optionally feed to hash
        // Overridden by TabletDebugExecutor and SingleDbScanner
        self.raw_scan_impl(from_key, end_key, limit, cf);
    }

    /// Compare two clusters by splitting the key range into fixed-size
    /// chunks, computing SHA256 hash for each chunk on both sides, and
    /// reporting mismatches.
    fn checksum(
        &self,
        other: &dyn DebugExecutor,
        from: &[u8],
        to: &[u8],
        chunk_mb: u64,
        cf: &str,
        ...
        // If no to-key specified, just hash the full from-key range
        if to.is_empty() {
            let mut src_hash = Sha256::new();
            let mut tgt_hash = Sha256::new();
            self.raw_scan_impl_with_hash(from, to, usize::MAX, cf, Some(&mut src_hash));
            other.raw_scan_impl_with_hash(from, to, usize::MAX, cf, Some(&mut tgt_hash));
            let src_h = hex::encode(src_hash.finalize());
            let tgt_h = hex::encode(tgt_hash.finalize());
            println!("Source hash: {}", src_h);
            println!("Target hash: {}", tgt_h);
            if src_h == tgt_h {
                println!("OK: checksum MATCH");
            } else {
                println!("MISMATCH");
            }
        }
    ...
    K: KvFormat,
{
    ...
    fn raw_scan_impl_with_hash(
        &self, from_key: &[u8], end_key: &[u8], _limit: usize, cf: &str,
        mut hash: Option<&mut sha2::Sha256>,
    ) {
        use sha2::Digest;
        let res = self
            .raw_scan(from_key, end_key, usize::MAX, cf)
            .unwrap_or_else(|e| perror_and_exit("Debugger::raw_scan", e));
        for (k, v) in &res {
...
/// Raw RocksDB C FFI for secondary instance access.
/// Enables opening a RocksDB in read-only mode without acquiring the LOCK
/// file, so we can scan tablets while TiKV is running.
mod rocksdb_secondary {
    use std::ffi::CStr;
    use std::os::raw::{c_char, c_void};
    use std::ptr;

    pub type DB = *mut c_void;
    pub type Options = *mut c_void;
    pub type ReadOptions = *mut c_void;
    pub type Iterator = *mut c_void;

    extern "C" {
        pub fn rocksdb_options_create() -> Options;
        pub fn rocksdb_options_destroy(opts: Options);
        pub fn rocksdb_options_set_create_if_missing(opts: Options, v: i32);
        ...
        pub fn rocksdb_close(db: DB);
        pub fn rocksdb_readoptions_create() -> ReadOptions;
        pub fn rocksdb_readoptions_destroy(opts: ReadOptions);
        pub fn rocksdb_create_iterator(db: DB, opts: ReadOptions) -> Iterator;
        pub fn rocksdb_iter_destroy(iter: Iterator);
        pub fn rocksdb_iter_valid(iter: Iterator) -> u8;
        pub fn rocksdb_iter_seek(iter: Iterator, key: *const u8, klen: usize);
        pub fn rocksdb_iter_seek_to_first(iter: Iterator);
        pub fn rocksdb_iter_key(iter: Iterator, klen: *mut usize) -> *const u8;
        pub fn rocksdb_iter_value(iter: Iterator, vlen: *mut usize) -> *const u8;
        pub fn rocksdb_iter_next(iter: Iterator);
        pub fn rocksdb_free(p: *mut c_void);
    }
    ...
    impl SecondaryDB {
        pub fn open(path: &std::path::Path) -> Result<Self, String> {
            ...
            let secondary_path = std::ffi::CString::new(
                format!("{}.secondary", path.to_str().unwrap())
            ).unwrap();
            ...
            unsafe {
                ...
                let db = rocksdb_open_as_secondary(
                    opts,
                    path_c.as_ptr(),
                ...
                if db.is_null() {
                    ...
                    if !err.is_null() { rocksdb_free(err as *mut c_void); }
                    return Err(err_msg);
                }
                Ok(SecondaryDB { db })
            }
        ...
        /// Try to hex-decode an ASCII hex string (e.g. "7480AABB").
        /// Returns Some(decoded_bytes) if the input is all hex chars, None otherwise.
        pub fn try_decode_hex(data: &[u8]) -> Option<Vec<u8>> {
            super::try_decode_hex(data)
        }

        pub fn scan(&self, from: &[u8], to: &[u8], limit: usize) -> Vec<(Vec<u8>, Vec<u8>)> {
            let mut results = Vec::new();
            // Auto-detect hex-encoded keys (e.g. from "--from 7480AABB")
            let from_bytes = Self::try_decode_hex(from).unwrap_or_else(|| from.to_vec());
            let to_bytes = Self::try_decode_hex(to).unwrap_or_else(|| to.to_vec());

            unsafe {
                let opts = rocksdb_readoptions_create();
                let iter = rocksdb_create_iterator(self.db, opts);
                rocksdb_readoptions_destroy(opts);

                if !from_bytes.is_empty() {
                    rocksdb_iter_seek(iter, from_bytes.as_ptr(), from_bytes.len());
                } else {
                    rocksdb_iter_seek_to_first(iter);
                }

                let mut count = 0u64;
                while rocksdb_iter_valid(iter) != 0 && results.len() < limit {
                    count += 1;
                    let mut klen: usize = 0;
                    let mut vlen: usize = 0;
                    let kptr = rocksdb_iter_key(iter, &mut klen);
                    let vptr = rocksdb_iter_value(iter, &mut vlen);
                    if !kptr.is_null() && !vptr.is_null() {
                        let key = std::slice::from_raw_parts(kptr, klen).to_vec();
                        if !to_bytes.is_empty() && key.as_slice() > to_bytes.as_slice() {
                            break;
                        }
                        let value = std::slice::from_raw_parts(vptr, vlen).to_vec();
                        results.push((key, value));
                    }
                    rocksdb_iter_next(iter);
                }
                rocksdb_iter_destroy(iter);
                eprintln!("SecondaryDB: scanned {} keys, kept {}", count, results.len());
            }
            results
        }
    }
    ...
    impl Drop for SecondaryDB {
        fn drop(&mut self) {
            unsafe { rocksdb_close(self.db); }
        }
    }
}

/// Online scanner for v1 (raft-kv) single RocksDB.
/// Uses RocksDB secondary instance to scan while TiKV is running.
struct SingleDbScanner {
    db_path: PathBuf,
}

impl SingleDbScanner {
    fn new(db_path: PathBuf) -> Self {
        println!("Single-DB scanner (secondary instance, TiKV running)");
        Self { db_path }
    }
}

impl DebugExecutor for SingleDbScanner {
    fn check_local_mode(&self) {}
    fn get_all_regions_in_store(&self) -> Vec<u64> { Vec::new() }
    fn get_value_by_key(&self, _: &str, _: Vec<u8>) -> Vec<u8> { unimplemented!() }
    fn get_region_size(&self, _: u64, _: Vec<&str>) -> Vec<(String, usize)> { unimplemented!() }
    fn get_region_info(&self, _: u64) -> RegionInfo { unimplemented!() }
    fn get_raft_log(&self, _: u64, _: u64) -> Entry { unimplemented!() }
    fn get_mvcc_infos(&self, _: Vec<u8>, _: Vec<u8>, _: u64) -> MvccInfoStream {
        Box::pin(stream::once(future::err("only raw_scan".to_string())))
    }

    fn raw_scan_impl(&self, from_key: &[u8], end_key: &[u8], limit: usize, _cf: &str) {
        use rocksdb_secondary::SecondaryDB;
        let db = match SecondaryDB::open(&self.db_path) {
            Ok(db) => db,
            Err(e) => { println!("Failed to open DB: {}", e); return; }
        };
        let results = db.scan(from_key, end_key, limit);
        for (k, v) in &results {
            println!("key: \"{}\", value: \"{}\"", escape(k), escape(v));
        }
        println!();
        println!("Total scanned keys: {}", results.len());
    }

    fn raw_scan_impl_with_hash(
        &self, from_key: &[u8], end_key: &[u8], _limit: usize, _cf: &str,
        mut hash: Option<&mut sha2::Sha256>,
    ) {
        use rocksdb_secondary::SecondaryDB;
        use sha2::Digest;
        ...
        for (k, v) in db.scan(from_key, end_key, usize::MAX) {
            if let Some(ref mut h) = hash {
                h.update(&k);
                h.update(&v);
            }
    ...
    fn check_region_consistency(&self, _: u64) {}
    fn modify_tikv_config(&self, _: &str, _: &str) {}
    fn dump_metrics(&self, _: Vec<&str>) {}
    fn dump_region_properties(&self, _: u64) {}
    fn dump_range_properties(&self, _start: Vec<u8>, _end: Vec<u8>) {}
    fn dump_store_info(&self) {}
    fn dump_cluster_info(&self) {}
    fn reset_to_version(&self, _: u64) {}
    fn get_region_read_progress(&self, _region_id: u64, _log: bool, _min_start_ts: u64) {}
    fn print_bad_regions(&self) {}
    fn remove_fail_stores(&self, _: Vec<u64>, _: Option<Vec<u64>>, _: bool) {}
    fn do_compaction(&self, _db: DbType, _cf: &str, _from: &[u8], _to: &[u8], _threads: u32, _bottommost: BottommostLevelCompaction) {}
    fn flashback_to_version(&self, _version: u64, _region_id: u64, _key_range: KeyRange, _start_ts: u64, _commit_ts: u64) -> Result<(), (KeyRange, grpcio::Error)> {
        Err((KeyRange::default(), grpcio::Error::RpcFailure(grpcio::RpcStatus::new(grpcio::RpcStatusCode::UNIMPLEMENTED))))
    }
}

struct TabletDebugExecutor {
    tablet_reg: TabletRegistry<RocksEngine>,
}

impl TabletDebugExecutor {
    fn new(tablet_reg: TabletRegistry<RocksEngine>) -> Self {
        println!("Tablet-only scanner (TiKV running, only raw_scan available)");
        Self { tablet_reg }
    }

    /// Scan all tablets using RocksDB secondary instances (no LOCK needed).
    fn raw_scan_tablets_secondary(&self, from_key: &[u8], end_key: &[u8], limit: usize) {
        use rocksdb_secondary::SecondaryDB;

        eprintln!("TABLET_SCAN: from_key len={} end_key len={}", from_key.len(), end_key.len());

        let tablet_root = self.tablet_reg.tablet_root();
        if !tablet_root.exists() {
            println!("Total scanned keys: 0");
            return;
        }

        let mut results = Vec::new();
        if let Ok(entries) = std::fs::read_dir(tablet_root) {
            for entry in entries.flatten() {
                if results.len() >= limit { break; }
                let path = entry.path();
                if !path.is_dir() { continue; }
                let dir_name = path.file_name().unwrap().to_str().unwrap_or("");
                if !dir_name.contains('_') { continue; }

                // Open as RocksDB secondary instance (bypasses LOCK)
                let db = match SecondaryDB::open(&path) {
                    Ok(db) => db,
                    Err(_) => continue,
                };
                let kvs = db.scan(from_key, end_key, limit - results.len());
                results.extend(kvs);
            }
        }
        results.sort_by(|a, b| a.0.cmp(&b.0));
        for (k, v) in &results {
            println!("key: \x22{}\x22, value: \x22{}\x22", escape(k), escape(v));
        }
        println!();
        println!("Total scanned keys: {}", results.len());
    }
...
impl DebugExecutor for TabletDebugExecutor {
    fn check_local_mode(&self) {}
    fn get_all_regions_in_store(&self) -> Vec<u64> { Vec::new() }
    fn get_value_by_key(&self, _cf: &str, _key: Vec<u8>) -> Vec<u8> { unimplemented!() }
    fn get_region_size(&self, _region: u64, _cfs: Vec<&str>) -> Vec<(String, usize)> { unimplemented!() }
    fn get_region_info(&self, _region: u64) -> RegionInfo { unimplemented!() }
    fn get_raft_log(&self, _region: u64, _index: u64) -> Entry { unimplemented!() }
    fn get_mvcc_infos(&self, _from: Vec<u8>, _to: Vec<u8>, _limit: u64) -> MvccInfoStream {
        Box::pin(stream::once(future::err("only raw_scan".to_string())))
    }

    fn raw_scan_impl(&self, from_key: &[u8], end_key: &[u8], limit: usize, _cf: &str) {
        self.raw_scan_tablets_secondary(from_key, end_key, limit);
    }
    ...
    fn raw_scan_impl_with_hash(
        ...
        eprintln!("TABLET_SCAN: from_key len={} end_key len={}", from_key.len(), end_key.len());
        let from_bytes = SecondaryDB::try_decode_hex(from_key).unwrap_or_else(|| from_key.to_vec());
        let to_bytes = SecondaryDB::try_decode_hex(end_key).unwrap_or_else(|| end_key.to_vec());

        let tablet_root = self.tablet_reg.tablet_root();
        if !tablet_root.exists() { return; }
        ...
        if let Ok(entries) = std::fs::read_dir(tablet_root) {
            for entry in entries.flatten() {
                ...
                if !path.is_dir() { continue; }
                let db = match SecondaryDB::open(&path) {
                    Ok(db) => db, Err(_) => continue,
                };
                for (k, v) in db.scan(&from_bytes, &to_bytes, usize::MAX) {
                    if let Some(ref mut h) = hash {
                        h.update(&k);
                        h.update(&v);
                    }
    ...
    fn remove_fail_stores(&self, _: Vec<u64>, _: Option<Vec<u64>>, _: bool) {}
    fn do_compaction(&self, _db: DbType, _cf: &str, _from: &[u8], _to: &[u8], _threads: u32, _bottommost: BottommostLevelCompaction) {}
    fn flashback_to_version(&self, _version: u64, _region_id: u64, _key_range: KeyRange, _start_ts: u64, _commit_ts: u64) -> Result<(), (KeyRange, grpcio::Error)> { Err((KeyRange::default(), grpcio::Error::RpcFailure(grpcio::RpcStatus::new(grpcio::RpcStatusCode::UNIMPLEMENTED)))) }
}
