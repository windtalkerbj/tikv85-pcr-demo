...
pub enum Cmd {
    ...
    /// Raw put a key-value pair (for PCR write-protection testing)
    RawPut {
        #[structopt(short = "k", long, help = RAW_KEY_HINT)]
        key: String,
        #[structopt(short = "v", long)]
        value: String,
    },
    /// Raw batch put for bulk writes (reads JSON array from stdin/file)
    RawBatchPut {
        #[structopt(short = "k", long)]
        count: u64,
        #[structopt(short = "p", long, default_value = "perf")]
        prefix: String,
    },
    /// Txn prewrite a key-value pair (for PCR write-protection testing)
    KvPrewrite {
        #[structopt(short = "k", long, help = RAW_KEY_HINT)]
        key: String,
        #[structopt(short = "v", long)]
    ...
    RawScan {
        ...
        #[structopt(long)]
        /// Compute SHA256 hash of all KV pairs instead of printing them
        hash: bool,
    ...
    /// Compare two TiKV data directories by key-range chunk hashes.
    /// Splits the key range into fixed-size chunks, computes SHA256
    /// hash of KV pairs in each chunk on both clusters, and reports
    /// mismatches. Used for PCR data consistency verification.
    Checksum {
        #[structopt(
            short = "f", long, default_value = "",
            help = RAW_KEY_HINT,
        )]
        from: String,

        #[structopt(
            short = "t", long, default_value = "",
            help = RAW_KEY_HINT,
        )]
        to: String,

        #[structopt(long, default_value = "10")]
        /// Chunk size in MB for key range splitting
        chunk_mb: u64,
        ...
        #[structopt(long)]
        /// data-dir of the second (target) TiKV
        to_data_dir: Option<String>,

        #[structopt(long)]
        /// config of the second (target) TiKV
        to_config: Option<String>,
    },
}
