...
use kvproto::{
    ...
    kvrpcpb::{KvPair, Mutation, Op, PrewriteRequest, RawBatchPutRequest, RawPutRequest, SplitRegionRequest},
...
fn main() {
    ...
    match cmd {
        ...
        Cmd::RawPut { key, value } => {
            let key = unescape(&key);
            let value = unescape(&value);
            raw_put_via_grpc(&opt.pd, &opt.host, &key, &value, Arc::clone(&mgr));
        }
        Cmd::RawBatchPut { count, prefix } => {
            raw_batch_put_via_grpc(&opt.host, count, &prefix, Arc::clone(&mgr));
        }
        Cmd::KvPrewrite { key, value } => {
            let key = unescape(&key);
            let value = unescape(&value);
            kv_prewrite_via_grpc(&opt.pd, &opt.host, &key, &value, Arc::clone(&mgr));
        }
        ...
        cmd => {
            ...
            match cmd {
                ...
                Cmd::RawScan {
                    ...
                    let to = unescape(&to);
                    if hash {
                        debug_executor.raw_scan_hash(&from, &to, limit, &cf);
                    } else {
                ...
                Cmd::Checksum {
                    ...
                    let target_executor =
                        new_debug_executor(&to_config, to_data_dir, None, Arc::clone(&mgr));
                    let from = unescape(&from);
                    let to = unescape(&to);
                    debug_executor.checksum(
                        &*target_executor, &from, &to, chunk_mb, &cf,
                    );
                }
                Cmd::Compact {
                    region,
...
fn raw_put_via_grpc(
    ...
    let addr = if let Some(h) = host {
        h.clone()
    } else if let Some(pd_str) = _pd {
        let pd_client = get_pd_rpc_client(Some(pd_str.clone()), Arc::clone(&mgr));
        match pd_client.get_region_info(key) {
            Ok(region) => region.leader
                .and_then(|leader| pd_client.get_store(leader.get_store_id()).ok())
                .map(|s| s.get_address().to_string()),
            _ => None,
        }.unwrap_or_else(|| "127.0.0.1:20161".to_string())
    } else {
        "127.0.0.1:20161".to_string()
    };
    ...
    req.set_value(value.to_vec());
    req.set_cf("default".to_string());

    match tikv_client.raw_put(&req) {
        Ok(resp) => {
            if !resp.get_error().is_empty() {
                eprintln!("raw_put FAILED: {}", resp.get_error());
                return;
            }
            println!("raw_put OK: key_len={} value_len={}", key.len(), value.len());
        }
        Err(e) => {
            eprintln!("raw_put gRPC error: {:?}", e);
        }
    }
}

fn raw_batch_put_via_grpc(host: &Option<String>, count: u64, prefix: &str, mgr: Arc<SecurityManager>) {
    let addr = host.clone().unwrap_or_else(|| "127.0.0.1:20161".to_string());
    let tikv_client = {
        let cb = ChannelBuilder::new(Arc::new(Environment::new(1)));
        let channel = mgr.connect(cb, &addr);
        TikvClient::new(channel)
    ...
    while sent < count {
        ...
        let pairs: Vec<KvPair> = (sent..end).map(|i| {
            let mut p = KvPair::default();
            let k = format!("{}_{}", prefix, i);
            let v = format!("v_{}", i);
            p.set_key(k.into_bytes());
            p.set_value(v.into_bytes());
            p
        }).collect();
        req.set_pairs(pairs.into());
        req.set_cf("default".to_string());
        match tikv_client.raw_batch_put(&req) {
            Ok(_) => sent = end,
            Err(e) => { eprintln!("batch_put error at {}: {:?}", sent, e); break; }
        }
        if sent % 5000 == 0 { eprintln!("  {} / {}", sent, count); }
    }
    let elapsed = start.elapsed().as_secs_f64();
    println!("Done: {} KVs in {:.1}s ({:.0} KV/s)", sent, elapsed, sent as f64 / elapsed.max(0.01));
}

fn kv_prewrite_via_grpc(
    _pd: &Option<String>,
    host: &Option<String>,
    key: &[u8],
    value: &[u8],
    ...
    // Use a dummy start_ts and a simple mutation
    let mut mutation = Mutation::default();
    mutation.set_op(Op::Put);
    mutation.set_key(key.to_vec());
    mutation.set_value(value.to_vec());
    req.mut_context().set_region_id(2);
    req.set_mutations(vec![mutation].into());
    req.set_primary_lock(key.to_vec());
    req.set_start_version(1);

    match tikv_client.kv_prewrite(&req) {
        Ok(resp) => {
            if !resp.get_errors().is_empty() {
                eprintln!("txn prewrite FAILED: {:?}", resp.get_errors());
            } else {
                println!("txn prewrite OK: key_len={} value_len={}", key.len(), value.len());
            }
        }
        Err(e) => {
            eprintln!("txn prewrite gRPC error: {:?}", e);
        }
    }
}
